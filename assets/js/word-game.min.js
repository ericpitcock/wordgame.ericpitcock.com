...
}),capture):adv.call(layer,type,callback,capture)}),"function"==typeof layer.onclick&&(oldOnClick=layer.onclick,layer.addEventListener("click",function(event){oldOnClick(event)},!1),layer.onclick=null)}}var deviceIsWindowsPhone=navigator.userAgent.indexOf("Windows Phone")>=0,deviceIsAndroid=navigator.userAgent.indexOf("Android")>0&&!deviceIsWindowsPhone,deviceIsIOS=/iP(ad|hone|od)/.test(navigator.userAgent)&&!deviceIsWindowsPhone,deviceIsIOS4=deviceIsIOS&&/OS 4_\d(_\d)?/.test(navigator.userAgent),deviceIsIOSWithBadTarget=deviceIsIOS&&/OS [6-7]_\d/.test(navigator.userAgent),deviceIsBlackBerry10=navigator.userAgent.indexOf("BB10")>0;FastClick.prototype.needsClick=function(target){switch(target.nodeName.toLowerCase()){case"button":case"select":case"textarea":if(target.disabled)return!0;break;case"input":if(deviceIsIOS&&"file"===target.type||target.disabled)return!0;break;case"label":case"iframe":case"video":return!0}return/\bneedsclick\b/.test(target.className)},FastClick.prototype.needsFocus=function(target){switch(target.nodeName.toLowerCase()){case"textarea":return!0;case"select":return!deviceIsAndroid;case"input":switch(target.type){case"button":case"checkbox":case"file":case"image":case"radio":case"submit":return!1}return!target.disabled&&!target.readOnly;default:return/\bneedsfocus\b/.test(target.className)}},FastClick.prototype.sendClick=function(targetElement,event){var clickEvent,touch;document.activeElement&&document.activeElement!==targetElement&&document.activeElement.blur(),touch=event.changedTouches[0],clickEvent=document.createEvent("MouseEvents"),clickEvent.initMouseEvent(this.determineEventType(targetElement),!0,!0,window,1,touch.screenX,touch.screenY,touch.clientX,touch.clientY,!1,!1,!1,!1,0,null),clickEvent.forwardedTouchEvent=!0,targetElement.dispatchEvent(clickEvent)},FastClick.prototype.determineEventType=function(targetElement){return deviceIsAndroid&&"select"===targetElement.tagName.toLowerCase()?"mousedown":"click"},FastClick.prototype.focus=function(targetElement){var length;deviceIsIOS&&targetElement.setSelectionRange&&0!==targetElement.type.indexOf("date")&&"time"!==targetElement.type&&"month"!==targetElement.type?(length=targetElement.value.length,targetElement.setSelectionRange(length,length)):targetElement.focus()},FastClick.prototype.updateScrollParent=function(targetElement){var scrollParent,parentElement;if(scrollParent=targetElement.fastClickScrollParent,!scrollParent||!scrollParent.contains(targetElement)){parentElement=targetElement;do{if(parentElement.scrollHeight>parentElement.offsetHeight){scrollParent=parentElement,targetElement.fastClickScrollParent=parentElement;break}parentElement=parentElement.parentElement}while(parentElement)}scrollParent&&(scrollParent.fastClickLastScrollTop=scrollParent.scrollTop)},FastClick.prototype.getTargetElementFromEventTarget=function(eventTarget){return eventTarget.nodeType===Node.TEXT_NODE?eventTarget.parentNode:eventTarget},FastClick.prototype.onTouchStart=function(event){var targetElement,touch,selection;if(event.targetTouches.length>1)return!0;if(targetElement=this.getTargetElementFromEventTarget(event.target),touch=event.targetTouches[0],deviceIsIOS){if(selection=window.getSelection(),selection.rangeCount&&!selection.isCollapsed)return!0;if(!deviceIsIOS4){if(touch.identifier&&touch.identifier===this.lastTouchIdentifier)return event.preventDefault(),!1;this.lastTouchIdentifier=touch.identifier,this.updateScrollParent(targetElement)}}return this.trackingClick=!0,this.trackingClickStart=event.timeStamp,this.targetElement=targetElement,this.touchStartX=touch.pageX,this.touchStartY=touch.pageY,event.timeStamp-this.lastClickTime<this.tapDelay&&event.preventDefault(),!0},FastClick.prototype.touchHasMoved=function(event){var touch=event.changedTouches[0],boundary=this.touchBoundary;return Math.abs(touch.pageX-this.touchStartX)>boundary||Math.abs(touch.pageY-this.touchStartY)>boundary?!0:!1},FastClick.prototype.onTouchMove=function(event){return this.trackingClick?((this.targetElement!==this.getTargetElementFromEventTarget(event.target)||this.touchHasMoved(event))&&(this.trackingClick=!1,this.targetElement=null),!0):!0},FastClick.prototype.findControl=function(labelElement){return void 0!==labelElement.control?labelElement.control:labelElement.htmlFor?document.getElementById(labelElement.htmlFor):labelElement.querySelector("button, input:not([type=hidden]), keygen, meter, output, progress, select, textarea")},FastClick.prototype.onTouchEnd=function(event){var forElement,trackingClickStart,targetTagName,scrollParent,touch,targetElement=this.targetElement;if(!this.trackingClick)return!0;if(event.timeStamp-this.lastClickTime<this.tapDelay)return this.cancelNextClick=!0,!0;if(event.timeStamp-this.trackingClickStart>this.tapTimeout)return!0;if(this.cancelNextClick=!1,this.lastClickTime=event.timeStamp,trackingClickStart=this.trackingClickStart,this.trackingClick=!1,this.trackingClickStart=0,deviceIsIOSWithBadTarget&&(touch=event.changedTouches[0],targetElement=document.elementFromPoint(touch.pageX-window.pageXOffset,touch.pageY-window.pageYOffset)||targetElement,targetElement.fastClickScrollParent=this.targetElement.fastClickScrollParent),targetTagName=targetElement.tagName.toLowerCase(),"label"===targetTagName){if(forElement=this.findControl(targetElement)){if(this.focus(targetElement),deviceIsAndroid)return!1;targetElement=forElement}}else if(this.needsFocus(targetElement))return event.timeStamp-trackingClickStart>100||deviceIsIOS&&window.top!==window&&"input"===targetTagName?(this.targetElement=null,!1):(this.focus(targetElement),this.sendClick(targetElement,event),deviceIsIOS&&"select"===targetTagName||(this.targetElement=null,event.preventDefault()),!1);return deviceIsIOS&&!deviceIsIOS4&&(scrollParent=targetElement.fastClickScrollParent,scrollParent&&scrollParent.fastClickLastScrollTop!==scrollParent.scrollTop)?!0:(this.needsClick(targetElement)||(event.preventDefault(),this.sendClick(targetElement,event)),!1)},FastClick.prototype.onTouchCancel=function(){this.trackingClick=!1,this.targetElement=null},FastClick.prototype.onMouse=function(event){return this.targetElement?event.forwardedTouchEvent?!0:event.cancelable&&(!this.needsClick(this.targetElement)||this.cancelNextClick)?(event.stopImmediatePropagation?event.stopImmediatePropagation():event.propagationStopped=!0,event.stopPropagation(),event.preventDefault(),!1):!0:!0},FastClick.prototype.onClick=function(event){var permitted;return this.trackingClick?(this.targetElement=null,this.trackingClick=!1,!0):"submit"===event.target.type&&0===event.detail?!0:(permitted=this.onMouse(event),permitted||(this.targetElement=null),permitted)},FastClick.prototype.destroy=function(){var layer=this.layer;deviceIsAndroid&&(layer.removeEventListener("mouseover",this.onMouse,!0),layer.removeEventListener("mousedown",this.onMouse,!0),layer.removeEventListener("mouseup",this.onMouse,!0)),layer.removeEventListener("click",this.onClick,!0),layer.removeEventListener("touchstart",this.onTouchStart,!1),layer.removeEventListener("touchmove",this.onTouchMove,!1),layer.removeEventListener("touchend",this.onTouchEnd,!1),layer.removeEventListener("touchcancel",this.onTouchCancel,!1)},FastClick.notNeeded=function(layer){var metaViewport,chromeVersion,blackberryVersion,firefoxVersion;if("undefined"==typeof window.ontouchstart)return!0;if(chromeVersion=+(/Chrome\/([0-9]+)/.exec(navigator.userAgent)||[,0])[1]){if(!deviceIsAndroid)return!0;if(metaViewport=document.querySelector("meta[name=viewport]")){if(-1!==metaViewport.content.indexOf("user-scalable=no"))return!0;if(chromeVersion>31&&document.documentElement.scrollWidth<=window.outerWidth)return!0}}if(deviceIsBlackBerry10&&(blackberryVersion=navigator.userAgent.match(/Version\/([0-9]*)\.([0-9]*)/),blackberryVersion[1]>=10&&blackberryVersion[2]>=3&&(metaViewport=document.querySelector("meta[name=viewport]")))){if(-1!==metaViewport.content.indexOf("user-scalable=no"))return!0;if(document.documentElement.scrollWidth<=window.outerWidth)return!0}return"none"===layer.style.msTouchAction||"manipulation"===layer.style.touchAction?!0:(firefoxVersion=+(/Firefox\/([0-9]+)/.exec(navigator.userAgent)||[,0])[1],firefoxVersion>=27&&(metaViewport=document.querySelector("meta[name=viewport]"),metaViewport&&(-1!==metaViewport.content.indexOf("user-scalable=no")||document.documentElement.scrollWidth<=window.outerWidth))?!0:"none"===layer.style.touchAction||"manipulation"===layer.style.touchAction?!0:!1)},FastClick.attach=function(layer,options){return new FastClick(layer,options)},"function"==typeof define&&"object"==typeof define.amd&&define.amd?define(function(){return FastClick}):"undefined"!=typeof module&&module.exports?(module.exports=FastClick.attach,module.exports.FastClick=FastClick):window.FastClick=FastClick}();var mobileKeys=!1,desktopKeys=!1,renderKeys=function renderKeys(){function keySort(order){keysContainer.find("div").sort(function(a,b){return+a.getAttribute(order)-+b.getAttribute(order)}).appendTo(keysContainer)}var keysContainer=$(".keys");return Modernizr.mq("(max-width: 768px)")?desktopKeys===!0&&(keySort("data-qwerty-order"),0===$('div[data-qwerty-order="10"] > span.break').length&&$('div[data-qwerty-order="10"], div[data-qwerty-order="19"]').after('<span class="break"></span>'),desktopKeys===!0&&($(".freebie-button").insertBefore('div[data-qwerty-order="20"]'),$(".skip-button").insertAfter('div[data-qwerty-order="26"]')),mobileKeys=!0,desktopKeys=!1):Modernizr.mq("(min-width: 769px)")&&(desktopKeys===!1&&(keySort("data-character-code"),keysContainer.find("span.break").remove(),$(".freebie-button").appendTo(".keys"),$(".skip-button").appendTo(".keys")),mobileKeys=!1,desktopKeys=!0),renderKeys}(),resizeMobileKeys=function resizeMobileKeys(){return $(".keys div:not(.special)").each(function(){$(this).css({"line-height":$(this).height()+"px"})}),resizeMobileKeys}();$(window).resize(function(){renderKeys(),resizeMobileKeys()}),function(){var WordGame={alertSound:new Audio("bleep.wav"),timeout:0,secretWord:"",characterCount:0,secretWordCharacterCodes:[],secretWordObject:{},uniqueLetters:0,definition:"",alternateDefinition:"",attemptsAllowed:0,attempts:0,attemptedLetters:[],attemptsLeft:0,lettersLeft:0,storedScore:window.localStorage.getItem("word-game-score"),naughtyWords:["skank","wetback","bitch","cunt","dick","douchebag","dyke","fag","nigger","tranny","trannies","paki","pussy","retard","slut","titt","tits","wop","whore","chink","fatass","shemale","daygo","dego","dago","gook","kike","kraut","spic","twat","lesbo","homo","fatso","lardass","jap","biatch","tard","gimp","gyp","chinaman","chinamen","golliwog","crip","raghead","negro","darky","hooker","honky","coolie","bastard","douche","penis","vagina","blowjob","popery","fuck","mulatto","faggot","jew"],settings:{sound:!1},animate:function(animation){$(".secret-word").addClass("animated "+animation),$(".secret-word").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){$(".secret-word").removeClass("animated "+animation)})},getSecretWord:function(){window.location.search?(debugWord=window.location.search,secretWord=debugWord.replace("?",""),WordGame.filterSecretWord(),console.log("DEBUG: "+secretWord)):$.ajax({type:"GET",url:"http://api.wordnik.com:80/v4/words.json/randomWord",data:{hasDictionaryDef:!0,excludePartOfSpeech:"family-name, given-name, noun-plural, proper-noun, proper-noun-plural, proper-noun-posessive, suffix",minCorpusCount:2e3,maxCorpusCount:-1,minDictionaryCount:3,maxDictionaryCount:-1,minLength:3,maxLength:7,api_key:"65bc764390b4030e69a110bbfb408a56d163ce85ef94ff62a"},success:function(data){void 0===data.word?WordGame.getSecretWord():(secretWord=data.word,WordGame.filterSecretWord())}})},filterSecretWord:function(){$.inArray(secretWord,this.naughtyWords)>-1?(console.log(secretWord+" is naughty, running again"),WordGame.getSecretWord()):isLettersOnly(secretWord)===!1?(console.log(secretWord+" has bad characters, running again"),WordGame.getSecretWord()):WordGame.getDefinition()},getDefinition:function(){$.ajax({type:"GET",url:"http://api.wordnik.com:80/v4/word.json/"+secretWord+"/definitions",data:{limit:2,includeRelated:!1,useCanonical:!0,includeTags:!1,api_key:"65bc764390b4030e69a110bbfb408a56d163ce85ef94ff62a"},success:function(data){definition=data[0].text,data[1]&&(this.alternateDefinition=data[1].text),WordGame.filterDefinition()}})},filterDefinition:function(){-1!=definition.toUpperCase().indexOf(secretWord.toUpperCase())?(console.log(secretWord+" is in "+definition+" (main def), running again"),WordGame.getSecretWord()):""!==this.alternateDefinition&&-1!=this.alternateDefinition.toUpperCase().indexOf(secretWord.toUpperCase())?(console.log(secretWord+" is in "+this.alternateDefinition+"(alt def), running again"),WordGame.getSecretWord()):(console.log("secret word: "+secretWord),console.log("main definition: "+definition),console.log(this.alternateDefinition?"alt definition: "+this.alternateDefinition:"alt definition: UNAVAILABLE"),WordGame.processSecretWord())},processSecretWord:function(){characterCount=secretWord.length;for(var index=0;index<characterCount;index++){var character=secretWord.charCodeAt(index);this.secretWordObject[character]?this.secretWordObject[character]++:this.secretWordObject[character]=1}console.log(this.secretWordObject),uniqueLetters=Object.keys(this.secretWordObject).length,attemptsAllowed=2*uniqueLetters,WordGame.renderUI()},renderUI:function(){for(var index=0;index<characterCount;index++){var charCodes=secretWord.charCodeAt(index);this.secretWordCharacterCodes.push(charCodes),$(".secret-word").append('<input class="letter-holder" readonly type="text" value="'+this.secretWordCharacterCodes[index]+'" />')}WordGame.animate("bounceInRight"),$(".attempts-left").html(attemptsAllowed),$(".definition p").html(definition);var scoreValue;null===window.localStorage.getItem("word-game-score")?(scoreValue="0",window.localStorage.setItem("word-game-score",0)):scoreValue=window.localStorage.getItem("word-game-score"),$(".score-value").text(scoreValue),""===this.alternateDefinition?$(".show-alternate-definition").css("visibility","hidden"):$(".show-alternate-definition").css("visibility","visible")},handleInput:function(characterCode,attempt){if(-1==$.inArray(characterCode,this.attemptedLetters)){attempt===!0&&this.attempts++,this.attemptedLetters.push(characterCode),$('div[data-character-code="'+characterCode+'"]').addClass("letter-selected");var attemptsLeft=attemptsAllowed-this.attempts,lettersLeft=Object.keys(this.secretWordObject).length;if($(".attempts-left").html(attemptsLeft),characterCode in this.secretWordObject){if($('input.letter-holder[value="'+characterCode+'"]').val(String.fromCharCode(characterCode)).addClass("highlight"),console.log(String.fromCharCode(characterCode)+" is a match, and appears "+this.secretWordObject[characterCode]+" time(s)"),delete this.secretWordObject[characterCode],$.isEmptyObject(this.secretWordObject)){console.log("YOU WIN"),WordGame.animate("flash"),this.attempts==uniqueLetters&&console.log("DUDE PERFECT!");var updatedScore,currentScore=parseInt(window.localStorage.getItem("word-game-score"))||0;updatedScore=$(".freebie-button").hasClass("letter-selected")?10*characterCount+5*attemptsLeft+currentScore:10*characterCount+5*attemptsLeft+5+currentScore,localStorage.setItem("word-game-score",updatedScore),$(".score-value").prop("number",currentScore).animateNumber({number:updatedScore}),WordGame.proceed("win")}else $.isEmptyObject(this.secretWordObject)||this.settings.sound!==!0||this.alertSound.play();console.log(this.secretWordObject)}else console.log(String.fromCharCode(characterCode)+" is NOT a match"),WordGame.animate("shake"),(lettersLeft>attemptsLeft||this.attempts==attemptsAllowed)&&(console.log("YOU LOSE"),WordGame.proceed("lose"));console.log("attempted letters: "+this.attemptedLetters),console.log("attempts: "+this.attempts+" / attempts left: "+attemptsLeft+" / unique letters left: "+lettersLeft)}else console.log("letter was already tried, doing nothing")},proceed:function(type){"win"!=type&&$("input.letter-holder:not(.highlight)").each(function(){$(this).val(String.fromCharCode($(this).val())).addClass("highlight")}),setTimeout(function(){console.clear(),$(".secret-word").addClass("animated bounceOutLeft"),$(".secret-word").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){$(".secret-word").removeClass("animated bounceOutLeft"),$(".secret-word, .definition p").empty(),$(".keys div").removeClass().removeAttr("class"),$(".freebie-button").removeAttr("disabled"),WordGame.secretWord="",WordGame.characterCount=0,WordGame.secretWordCharacterCodes=[],WordGame.secretWordObject={},WordGame.uniqueLetters=0,WordGame.definition="",WordGame.alternateDefinition="",WordGame.attemptsAllowed=0,WordGame.attempts=0,WordGame.attemptedLetters=[],WordGame.attemptsLeft=0,WordGame.lettersLeft=0,WordGame.getSecretWord()})},1300)}};WordGame.getSecretWord(),$(function(){FastClick.attach(document.body)}),document.body.addEventListener("touchmove",function(e){e.preventDefault()},!1),$(".skip-button").click(function(){WordGame.proceed("skip")}),$(".freebie-button").click(function(){if(!$(this).hasClass("letter-selected")){var unusedLetters=Object.keys(WordGame.secretWordObject),randomUnusedLetter=unusedLetters[Math.floor(Math.random()*unusedLetters.length)];WordGame.handleInput(randomUnusedLetter,!1),$(this).addClass("letter-selected")}}),$(document).keypress(function(e){function keyPressed(){var characterCode=e.which;if(13==characterCode)WordGame.proceed("skip");else{if(!(characterCode>=97&&122>=characterCode))return;WordGame.handleInput(characterCode,!0)}}e.preventDefault(),clearTimeout(WordGame.timeout),WordGame.timeout=setTimeout(keyPressed,50)}),$(".keys div:not(.special)").click(function(e){e.preventDefault(),$(this).hasClass("letter-selected")||WordGame.handleInput($(this).data("character-code"),!0)}),$("input[name=sound]:radio").change(function(){"on"==this.value?WordGame.settings.sound=!0:"off"==this.value&&(WordGame.settings.sound=!1)})}();