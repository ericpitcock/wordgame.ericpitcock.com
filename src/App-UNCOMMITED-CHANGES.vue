<template>
  <div id="app" :style="{ background: 'hsla(145, 50%, ' + backgroundLightness + '%, 1)' }">
    <div class="title">
    ...
    <div class="selections">
      ...
      <div v-for="letter in keys" :class="{ nope: incorrectLetters.includes(letter), yep: correctLetters.includes(letter) }">{{ letter }}</div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      keys() { return this.alphabet } // need toggle between this.alphabet and this.qwerty based on viewport
    },
    methods: {
      uniqueLetters() {
      ...
      checkWordForLetter(code) {
        ...
        if (this.secretWord.array.includes(letter)) {
          this.correctLetters.push(this.getLetter(code))
